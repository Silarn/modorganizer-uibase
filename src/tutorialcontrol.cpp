...
TutorialControl::TutorialControl(QWidget* targetControl, const QString& name)
    : QObject(nullptr), m_TargetControl(targetControl), m_Name(name), m_TutorialView(nullptr),
      m_Manager(TutorialManager::instance()), m_ExpectedTab(0), m_CurrentClickControl(nullptr) {}
...
void TutorialControl::registerControl() { m_Manager.registerControl(m_Name, this); }
...
void TutorialControl::startTutorial(const QString& tutorial) {
    if (m_TutorialView == nullptr) {
        ...
        QString qmlName = canonicalPath(QCoreApplication::applicationDirPath() + "/tutorials") + "/tutorials_" +
                          m_Name.toLower() + ".qml";
...
void TutorialControl::simulateClick(int x, int y) {
    ...
    QMouseEvent* downEvent =
        new QMouseEvent(QEvent::MouseButtonPress, hitPos, Qt::LeftButton, Qt::LeftButton, Qt::NoModifier);
    QMouseEvent* upEvent =
        new QMouseEvent(QEvent::MouseButtonRelease, hitPos, Qt::LeftButton, Qt::LeftButton, Qt::NoModifier);
...
void TutorialControl::nextTutorialStepProxy() {
    ...
    if (m_TutorialView != nullptr) {
        ...
        if (sender()->inherits("QAction")) {
            success = disconnect(sender(), SIGNAL(triggered()), this, SLOT(nextTutorialStepProxy()));
        } else {
            success = disconnect(sender(), SIGNAL(pressed()), this, SLOT(nextTutorialStepProxy()));
        }
        if (!success) {
...
bool TutorialControl::waitForAction(const QString& actionName) {
    if (m_TargetControl != nullptr) {
        ...
        if (action == nullptr) {
            qCritical("no action \"%s\" in control \"%s\"", actionName.toUtf8().constData(),
...
bool TutorialControl::waitForButton(const QString& buttonName) {
    if (m_TargetControl != nullptr) {
        ...
        if (button == nullptr) {
            qCritical("no button \"%s\" in control \"%s\"", buttonName.toUtf8().constData(),
                      m_Name.toUtf8().constData());
            return false;
        }
        if (button->isEnabled()) {
...
bool TutorialControl::waitForTabOpen(const QString& tabControlName, int tab) {
    if (m_TargetControl != nullptr) {
        ...
        if (tabWidget == nullptr) {
            qCritical("no tab widget \"%s\" in control \"%s\"", tabControlName.toUtf8().constData(),
                      m_Name.toUtf8().constData());
            return false;
        }

...
MOBase::ModRepositoryFileInfo::ModRepositoryFileInfo(const ModRepositoryFileInfo& reference)
    : QObject(reference.parent()), name(reference.name), uri(reference.uri), description(reference.description),
      version(reference.version), categoryID(reference.categoryID), modName(reference.modName), modID(reference.modID),
      fileID(reference.fileID), fileSize(reference.fileSize), fileCategory(reference.fileCategory),
...
MOBase::ModRepositoryFileInfo::ModRepositoryFileInfo(int modID, int fileID)
    : name(), uri(), description(), version(), categoryID(0), modName(), modID(modID), fileID(fileID), fileSize(0),

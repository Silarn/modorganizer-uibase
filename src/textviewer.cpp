...
TextViewer::~TextViewer() { delete ui; }
    ...
    if (!m_Modified.empty()) {
        for (std::set<QTextEdit*>::iterator iter = m_Modified.begin(); iter != m_Modified.end(); ++iter) {
            QMessageBox::StandardButton res = QMessageBox::question(
                this, tr("Save changes?"), tr("Do you want to save changes to %1?").arg((*iter)->documentTitle()),
...
void TextViewer::patternChanged(QString newPattern) { m_FindPattern = newPattern; }

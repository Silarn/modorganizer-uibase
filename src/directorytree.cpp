...
bool operator<(const FileTreeInformation& LHS, const FileTreeInformation& RHS) { return LHS.m_Name < RHS.m_Name; }

bool operator<(const DirectoryTreeInformation& LHS, const DirectoryTreeInformation& RHS) { return LHS.name < RHS.name; }
...
QString DirectoryTree::getFullPath(FileTreeInformation const* leaf) const {
    ...
    while (parent != nullptr) {
        if ((parent->getParent() != nullptr) && !result.isEmpty()) {
            result.prepend("\\");
        }

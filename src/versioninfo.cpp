...
VersionInfo::VersionInfo(const QString& versionString, VersionScheme scheme)
    : m_Valid(true), m_ReleaseType(RELEASE_FINAL), m_Major(0), m_Minor(0), m_SubMinor(0), m_SubSubMinor(0),
...
QString VersionInfo::displayString() const {
    ...
    if (m_Scheme == SCHEME_REGULAR) {
        ...
        result = QString("%1-%2-%3")
                     .arg(m_Major)
                     .arg(QString("%1").arg(m_Minor).rightJustified(2, '0'))
                     .arg(QString("%1").arg(m_SubMinor).rightJustified(2, '0'));
    }
    switch (m_ReleaseType) {
    case RELEASE_PREALPHA: {
        result.append(" pre-alpha");
...
QString VersionInfo::parseReleaseType(QString versionString) {
    ...
    static std::map<QString, ReleaseType> typeStrings = {
        {"prealpha", RELEASE_PREALPHA}, {"alpha", RELEASE_ALPHA}, {"beta", RELEASE_BETA}, {"rc", RELEASE_CANDIDATE}};
    ...
    if (m_Scheme == SCHEME_REGULAR) {
        // also interpret the a/b letters, but only if they follow immediately on the version number, otherwise the
        // margin for error is too big
...
void VersionInfo::parse(const QString& versionString, VersionScheme scheme, bool manualInput) {
    m_Valid = false;
    m_Scheme = scheme == SCHEME_LITERAL ? SCHEME_REGULAR : scheme != SCHEME_DISCOVER ? scheme : SCHEME_REGULAR;
    m_ReleaseType = RELEASE_FINAL;
...
bool operator<(const VersionInfo& LHS, const VersionInfo& RHS) {
    ...
    if ((LHS.m_Scheme == VersionInfo::SCHEME_DATE) && (RHS.m_Scheme != VersionInfo::SCHEME_DATE)) {
        return true;
    } else if ((LHS.m_Scheme != VersionInfo::SCHEME_DATE) && (RHS.m_Scheme == VersionInfo::SCHEME_DATE)) {
        return false;
    } else if ((LHS.m_Scheme == VersionInfo::SCHEME_DECIMALMARK) || (RHS.m_Scheme == VersionInfo::SCHEME_DECIMALMARK)) {
        // use decimal versioning if either version is a decimal. The parser interprets versions as regular if in doubt
        // so if the scheme is "decimal" it is definitively a decimal version number whereas SCHEME_REGULAR means
        // "probably regular"

        float leftVal = QString("%1.%2")
                            .arg(LHS.m_Major)
                            .arg(QString("%1").arg(LHS.m_Minor).rightJustified(LHS.m_DecimalPositions, '0'))
                            .toFloat();
        float rightVal = QString("%1.%2")
                             .arg(RHS.m_Major)
                             .arg(QString("%1").arg(RHS.m_Minor).rightJustified(RHS.m_DecimalPositions, '0'))
                             .toFloat();

...
class MyTree {
  ...
  private:
    struct ByNodeData {
        bool operator()(const Node* lhs, const Node* rhs) const { return lhs->getData() < rhs->getData(); }
    };
  ...
  public:
     ...
     **/
    node_iterator nodeFind(const NodeData& data) {
        Node temp;
        temp.setData(data);
        return m_Nodes.find(&temp);
    }

    /**
     * @return iterator to node with the specified data
     **/
    const_node_iterator nodeFind(const NodeData& data) const {
        Node temp;
        temp.setData(data);
        return m_Nodes.find(&temp);
    }

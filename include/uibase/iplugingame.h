...
class IPluginGame : public QObject, public IPlugin {
  ...
  public:
    ...
    enum ProfileSetting { MODS = 0x01, CONFIGURATION = 0x02, SAVEGAMES = 0x04, PREFER_DEFAULTS = 0x08 };
  ...
  public:
     ...
     * @note the MO app does not yet support virtualizing only specific aspects but plugins should be written with this
     * future functionality in mind
     * @note this function will be used to initially create a profile, potentially to repair it or upgrade/downgrade it
     * so the implementations have to gracefully handle the case that the directory already contains files!
     */

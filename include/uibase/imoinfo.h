...
class IOrganizer {
  ...
  public:
     ...
     * unless you have a really good reason to access settings of another mod. You can not access settings for a plugin
     ...
     * @param pluginName name of the plugin for which to change a value. This should always be IPlugin::name() unless
     * you have a really good reason to access data of another mod AND if you can verify that plugin is actually
     * installed
     * @param key identifier of the setting
     ...
     * @param pluginName name of the plugin for which to retrieve a value. This should always be IPlugin::name() unless
     * you have a really good reason to access data of another mod.
     ...
     *       main application. There is no UI for the user to change this value but (s)he can directly access the
     * storage
     */
     ...
     * @param key identifier of the value
     * @param value value to set
     * @param sync if true the storage is immediately written to disc. This costs performance but is safer against data
     * loss
     * @throw an exception is thrown if pluginName doesn't refer to an installed plugin
     */
    virtual void setPersistent(const QString& pluginName, const QString& key, const QVariant& value,
                               bool sync = true) = 0;
     ...
     * @param executable name or path of the executable. If this is only a filename it will only work if it has been
     * configured in MO as an executable. if it is a relative path it is expected to be relative to the game directory.
     * @param args arguments to pass to the executable. If this is empty and executable refers to a configured exe the
     * configured arguments are used
     * @param cwd working directory for the executable. If this is empty the path to the executable is used unless
     * executable referred to a configured MO executable. In that case the configured cwd is used
     ...
     * @note the returned handle is usually a job handle but under some situations MO may not be able to create a job
     * and then the handle is a process handle. Use waitForApplication to wait for completion on this handle, it works
     * for both process and job. If you want to write waiting code yourself, be aware that you can't
     * WaitForSingleObject on a job object. There is no single windows api call that works for both
     */
    virtual HANDLE startApplication(const QString& executable, const QStringList& args = QStringList(),
                                    const QString& cwd = "", const QString& profile = "") = 0;
     ...
     * @note the exit code is slightly fickle if handle is a job handle since there is no reliable way to know for which
     * process the exitcode was returned. If, at the end of the jobs lifetime only one process was left (for at least
     * 500ms) that processes exitcode will be returned. If there is only one process, again, the expected exit

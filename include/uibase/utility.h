 ...
 **/
QDLLEXPORT bool shellCopy(const QStringList& sourceNames, const QStringList& destinationNames,
 ...
 *                        There has to be one destination name for each source name or a single directory
 * @param yesToAll if true, the operation will assume "yes" to overwrite confirmations. This doesn't seem to work when
 *providing multiple files to copy
 * @param dialog a dialog to be the parent of possible confirmation dialogs
 * @return true on success, false on error. Call ::GetLastError() to retrieve error code
 **/
QDLLEXPORT bool shellCopy(const QString& sourceNames, const QString& destinationNames, bool yesToAll = false,
                          QWidget* dialog = nullptr);
 ...
 * @return true on success, false on error. Call ::GetLastError() to retrieve error code
 **/
QDLLEXPORT bool shellMove(const QStringList& sourceNames, const QStringList& destinationNames,
                          QWidget* dialog = nullptr);
 ...
 * @return true on success, false on error. Call ::GetLastError() to retrieve error code
 **/
QDLLEXPORT bool shellMove(const QString& sourceNames, const QString& destinationNames, bool yesToAll = false,
                          QWidget* dialog = nullptr);
 ...
 * @return true on success, false on error. Call ::GetLastError() to retrieve error code
 **/
QDLLEXPORT bool shellRename(const QString& oldName, const QString& newName, bool yesToAll = false,
                            QWidget* dialog = nullptr);
 ...
 * @note this is a workaround for win 8 and newer where shell operations caused the windows to loose focus even if no
 *dialog is shown
 **/
 ...
 * @param maximum maximum number of elements to print. If there are more elements, "..." is appended to the string
